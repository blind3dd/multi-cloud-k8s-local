...
# Install Kubernetes tools for karpenter-worker-1 (aws, talos)
set -euo pipefail

echo "Installing Kubernetes tools for karpenter-worker-1..."
...
if [[ "karpenter-worker-1" == karpenter-worker-* ]]; then
    echo "Installing Karpenter tools..."
    nix-env -iA nixpkgs.karpenter
fi

echo "Kubernetes tools installed for karpenter-worker-1"
