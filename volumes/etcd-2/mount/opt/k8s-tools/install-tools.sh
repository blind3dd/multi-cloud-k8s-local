...
# Install Kubernetes tools for etcd-2 (azure, flatcar)
set -euo pipefail

echo "Installing Kubernetes tools for etcd-2..."
...
if [[ "etcd-2" == karpenter-worker-* ]]; then
    echo "Installing Karpenter tools..."
    nix-env -iA nixpkgs.karpenter
fi

echo "Kubernetes tools installed for etcd-2"
