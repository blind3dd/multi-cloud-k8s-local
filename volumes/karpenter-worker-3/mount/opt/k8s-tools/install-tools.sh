...
# Install Kubernetes tools for karpenter-worker-3 (gcp, talos)
set -euo pipefail

echo "Installing Kubernetes tools for karpenter-worker-3..."
...
if [[ "karpenter-worker-3" == karpenter-worker-* ]]; then
    echo "Installing Karpenter tools..."
    nix-env -iA nixpkgs.karpenter
fi

echo "Kubernetes tools installed for karpenter-worker-3"
