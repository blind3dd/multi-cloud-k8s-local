...
# Install Kubernetes tools for etcd-3 (gcp, flatcar)
set -euo pipefail

echo "Installing Kubernetes tools for etcd-3..."
...
if [[ "etcd-3" == karpenter-worker-* ]]; then
    echo "Installing Karpenter tools..."
    nix-env -iA nixpkgs.karpenter
fi

echo "Kubernetes tools installed for etcd-3"
