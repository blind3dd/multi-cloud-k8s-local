...
# Install Kubernetes tools for talos-control-plane-3 (gcp, talos)
set -euo pipefail

echo "Installing Kubernetes tools for talos-control-plane-3..."
...
if [[ "talos-control-plane-3" == karpenter-worker-* ]]; then
    echo "Installing Karpenter tools..."
    nix-env -iA nixpkgs.karpenter
fi

echo "Kubernetes tools installed for talos-control-plane-3"
