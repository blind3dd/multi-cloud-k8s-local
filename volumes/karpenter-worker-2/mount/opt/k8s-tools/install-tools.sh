...
# Install Kubernetes tools for karpenter-worker-2 (azure, talos)
set -euo pipefail

echo "Installing Kubernetes tools for karpenter-worker-2..."
...
if [[ "karpenter-worker-2" == karpenter-worker-* ]]; then
    echo "Installing Karpenter tools..."
    nix-env -iA nixpkgs.karpenter
fi

echo "Kubernetes tools installed for karpenter-worker-2"
