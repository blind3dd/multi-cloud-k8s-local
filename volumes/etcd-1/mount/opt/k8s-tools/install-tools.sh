...
# Install Kubernetes tools for etcd-1 (aws, flatcar)
set -euo pipefail

echo "Installing Kubernetes tools for etcd-1..."
...
if [[ "etcd-1" == karpenter-worker-* ]]; then
    echo "Installing Karpenter tools..."
    nix-env -iA nixpkgs.karpenter
fi

echo "Kubernetes tools installed for etcd-1"
