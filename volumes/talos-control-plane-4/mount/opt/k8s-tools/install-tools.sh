...
# Install Kubernetes tools for talos-control-plane-4 (ibm, talos)
set -euo pipefail

echo "Installing Kubernetes tools for talos-control-plane-4..."
...
if [[ "talos-control-plane-4" == karpenter-worker-* ]]; then
    echo "Installing Karpenter tools..."
    nix-env -iA nixpkgs.karpenter
fi

echo "Kubernetes tools installed for talos-control-plane-4"
