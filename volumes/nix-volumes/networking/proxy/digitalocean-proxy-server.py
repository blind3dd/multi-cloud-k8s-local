...
class UdigitaloceanProxyHandler(BaseHTTPRequestHandler):
...
def main():
    ...
    server = HTTPServer(('0.0.0.0', port), UdigitaloceanProxyHandler)
    print(f"digitalocean proxy server starting on port {port}")
    
    try:
        server.serve_forever()
    except KeyboardInterrupt:
        print(f"\nShutting down digitalocean proxy server...")
        server.shutdown()
