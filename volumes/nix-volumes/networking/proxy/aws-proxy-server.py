...
class UawsProxyHandler(BaseHTTPRequestHandler):
...
def main():
    ...
    server = HTTPServer(('0.0.0.0', port), UawsProxyHandler)
    print(f"aws proxy server starting on port {port}")
    
    try:
        server.serve_forever()
    except KeyboardInterrupt:
        print(f"\nShutting down aws proxy server...")
        server.shutdown()
