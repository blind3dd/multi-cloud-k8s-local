...
# Install Kubernetes tools for karpenter-worker-5 (digitalocean, talos)
set -euo pipefail

echo "Installing Kubernetes tools for karpenter-worker-5..."
...
if [[ "karpenter-worker-5" == karpenter-worker-* ]]; then
    echo "Installing Karpenter tools..."
    nix-env -iA nixpkgs.karpenter
fi

echo "Kubernetes tools installed for karpenter-worker-5"
