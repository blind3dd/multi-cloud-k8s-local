...
# Install Kubernetes tools for karpenter-worker-4 (ibm, talos)
set -euo pipefail

echo "Installing Kubernetes tools for karpenter-worker-4..."
...
if [[ "karpenter-worker-4" == karpenter-worker-* ]]; then
    echo "Installing Karpenter tools..."
    nix-env -iA nixpkgs.karpenter
fi

echo "Kubernetes tools installed for karpenter-worker-4"
