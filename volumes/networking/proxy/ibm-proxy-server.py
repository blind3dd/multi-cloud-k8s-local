...
class UibmProxyHandler(BaseHTTPRequestHandler):
...
def main():
    ...
    server = HTTPServer(('0.0.0.0', port), UibmProxyHandler)
    print(f"ibm proxy server starting on port {port}")
    
    try:
        server.serve_forever()
    except KeyboardInterrupt:
        print(f"\nShutting down ibm proxy server...")
        server.shutdown()
