...
class UazureProxyHandler(BaseHTTPRequestHandler):
...
def main():
    ...
    server = HTTPServer(('0.0.0.0', port), UazureProxyHandler)
    print(f"azure proxy server starting on port {port}")
    
    try:
        server.serve_forever()
    except KeyboardInterrupt:
        print(f"\nShutting down azure proxy server...")
        server.shutdown()
