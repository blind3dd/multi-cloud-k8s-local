...
if nix-env -iA "nixpkgs.$PACKAGE_ATTR"; then
...
BINARY_PATH=$(which "$PACKAGE_NAME" 2>/dev/null || echo "")
...
PACKAGE_INFO=$(nix-env -q --out-path "$PACKAGE_NAME" 2>/dev/null || echo "")
if [ -n "$PACKAGE_INFO" ]; then
    print_status $BLUE "Package info: $PACKAGE_INFO"
    NIX_STORE_PATH=$(echo "$PACKAGE_INFO" | cut -d' ' -f2)
    print_status $GREEN "✅ Package information retrieved"
else
    print_error "Could not get package information for $PACKAGE_NAME"
fi

# Step 7: Verify Nix store path integrity
print_header "🛡️ Verifying Nix store path integrity"
if nix-store --verify-path "$NIX_STORE_PATH"; then
    print_status $GREEN "✅ Store path verification passed"
else
    print_error "Store path verification failed"
fi
...
if [ -n "$BINARY_PATH" ]; then
    ...
    if [ -f "$BINARY_PATH" ]; then
        ...
        sha256sum "$BINARY_PATH" 2>/dev/null || print_status $YELLOW "Could not calculate SHA256"
    fi
